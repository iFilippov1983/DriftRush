...
namespace Navigation
{
    public class Path : MonoBehaviour 
    {
        ...
        public Waypoint GetWaypointNextTo(Waypoint point)
        { 
            int wpIndex = new List<Waypoint>(_waypoints).IndexOf(point);
            if(wpIndex < _waypoints.Length - 1)
                return _waypoints[wpIndex + 1];
            else 
                return null;
        }
